...
function renderTrendPB(cross, chart, dArr, row, p) {
  ...
  // ---- SISTEMA COMPLETO (S17): il PB segna la direzione, il LY la corregge, 4 rafforzativi ----
  // Il PB da solo rende 53,5%. Con il correttivo LY il sistema rende 57,4% (z 9,50) sulle 4.111 carte.
  window.__lyCombo = null;
  if (signal === 'LONG' || signal === 'SHORT') {
    try {
      var _yb = (chart.source && chart.source.yearPillar) ? chart.source.yearPillar.charAt(1) : null;
      var _ly = pbManual
        ? window.XKDGLiuYao.readManual(pbManual.sup, pbManual.inf, pbManual.linea,
            chart.dayBranch, chart.monthBranch, _yb, chart.dayStem || null)
        : window.XKDGLiuYao.read(row.seed, chart.dayBranch, chart.monthBranch, _yb, chart.dayStem || null);
      if (_ly && !_ly.error) {
        var _ctxLY = {
          oraBranch: window.XKDGLiuYao.oraDalSeme(row.seed),
          emaDir: dir,
          date: (typeof forexData !== 'undefined' && forexData && forexData.date) ? forexData.date : null,
          corpoEl: pb.trend ? pb.trend.el : null,
          dayBranch: chart.dayBranch, monthBranch: chart.monthBranch,
          yearBranch: _yb, dayStem: chart.dayStem || null
        };
        var _c = window.XKDGLiuYao.combinaS9(_ly, _ctxLY, signal, {}, {}, {});
        if (_c && _c.finale) { window.__lyCombo = _c; signal = _c.finale; }
      }
    } catch (e) { window.__lyCombo = null; }
  }
  ...
  // riga del correttivo LY (sistema completo S17)
  var lyLine = '';
  if (window.__lyCombo) {
    var _cc = window.__lyCombo;
    var _pbSolo = (dir === 'up') ? (confirmed ? 'LONG' : 'SHORT') : (confirmed ? 'SHORT' : 'LONG');
    var _cambiato = (_cc.finale !== _pbSolo);
    lyLine = '<div class="trendmsgs">六爻 Liu Yao corrective: <b class="' +
      (_cambiato ? 'down' : 'up') + '">' + (_cambiato ? 'the LY OVERTURNS the PB' : 'confirms the PB') +
      '</b> → <b>' + _cc.finale + '</b><br><small style="opacity:.8">' + (_cc.chi || '') + '</small></div>';
  }
  ...
  p.innerHTML = head + seedLine + emaLine + lyLine + hexBlock + roles + manualRow;
