...
function leggi(seed, dayBranch, monthBranch, yearBranch, dayStem){
  ...
  const oraVuota = vuoti.indexOf(oraBranch) >= 0;
  ...
  let palazzoYong = HOUTIAN[usoNum];
  if (palazzoYong.length === 2) {
    const lineaYang = (linea % 2) === 1;
    palazzoYong = palazzoYong.filter(b => ((B.indexOf(b) % 2) === 0) === lineaYang);
  }
  // La regola vale SOLO se c'e' un trigramma vittorioso da legare, cioe' quando il
  // Trend controlla il Yong (我剋). Negli altri casi nessuno vince e non c'e' niente
  // da bloccare.  LARGO=1 la rimette su tutte le relazioni, per confronto.
  const cIsVincitore = process.env.LARGO ? true : (via === '我剋');
  const bloccato = !!process.env.BLOCCO && cIsVincitore &&
    palazzo.some(pt => palazzoYong.some(py => COMBINA[pt] === py));
  ...
  // il Yong occupa anch'esso un palazzo Houtian; se i rami sono due e' la LINEA MOBILE
  // a dire quale agisce (linee 1,3,5 = yang).  Se quel palazzo e' dell'elemento che il
  // Yong genera e che a sua volta genera il Trend, il controllo del Yong non arriva:
  // esce gia' trasformato, passa dentro il proprio palazzo e diventa nutrimento.
  // Il Trend regge e PROSEGUE.
  // E' l'unica regola che puo' portare un INVERTE a PROSEGUE.
  // PONTEY=solo剋我 (difetto) applica la regola solo dove il Yong controlla il Trend
  // PONTEY=largo la applica a tutte le relazioni con verdetto di base INVERTE
  const PY = process.env.NOPONTEY ? null : 'solo';
  const ponteYong = !!PY && via === '剋我' &&
    palazzoYong.some(py => GEN[uso.el] === WX[py] && GEN[WX[py]] === corpo.el);

  const spazzato = !eccitato && att > 0 && att > dif;
  return { via, base, finale: ponteYong ? true : ((spazzato || bloccato || autopen) ? false : base), spazzato,
           corpo, uso, sup, inf, linea, palazzo, palazzoYong, bloccato, autopen, ponteYong, oraVuota, vuoti, att, dif, dettAtt, dettDif, dettPonte, dettComb, eccitato, statoTrend, monthEl, oraBranch };
}
...
if (process.env.CARTA) {
  ...
  console.log('ponte del palazzo del Yong: '+(r.ponteYong?'SÌ, il controllo non arriva → il Trend prosegue':'no'));
...
Object.keys(hist.crosses).forEach(cross => {
  ...
  for (let k=12;k<days.length;k++){
    ...
    rows.push({cross,date:d,move,emaDir:ema.direction,via:r.via,
               base:r.base, finale:r.finale, spazzato:r.spazzato, bloccato:r.bloccato, autopen:r.autopen, ponteYong:r.ponteYong,
...
console.log('\npeggiori 10 carte');
rows.slice().sort((a,b)=>a.pnl-b.pnl).slice(0,10).forEach(r=>
  console.log('  '+r.date.split('-').reverse().join('/')+' '+r.cross+'  '+r.via+'  '+r.pnl.toFixed(0)+' pip'+(r.spazzato?'  (spazzato)':'')));

{
  const b = rows.filter(r=>r.ponteYong && r.base===false);
  if (b.length) console.log('\ncarte ribaltate dal ponte del palazzo del Yong: '+b.length+
    '   '+b.filter(r=>r.pnl>0).length+' giuste, '+b.filter(r=>r.pnl<0).length+' sbagliate, '+
    b.reduce((s,r)=>s+r.pnl,0).toFixed(0)+' pip');
}
