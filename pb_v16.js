...
function leggi(seed, dayBranch, monthBranch, yearBranch, dayStem){
  ...
  return { via, base, finale: (spazzato || bloccato || autopen) ? false : base, spazzato,
           corpo, uso, sup, inf, linea, palazzo, palazzoYong, bloccato, autopen, oraVuota, vuoti, att, dif, dettAtt, dettDif, dettPonte, dettComb, eccitato, statoTrend, monthEl, oraBranch };
}
...
Object.keys(hist.crosses).forEach(cross => {
  ...
  for (let k=12;k<days.length;k++){
    ...
    rows.push({cross,date:d,move,emaDir:ema.direction,via:r.via,
               base:r.base, finale:r.finale, spazzato:r.spazzato, bloccato:r.bloccato, autopen:r.autopen,
